...
const selectClass =
  "h-[38px] rounded-md border border-vega-border bg-[#0b141f] px-3 text-xs text-vega-text outline-none focus:border-vega-accent/70";
...
function roleClass(role: LoginRole | string) {
  if (role === "admin") return "border-vega-accent-border bg-vega-accent-soft text-[#93c5fd]";
...
function statusClass(status: UserStatus) {
  ...
  if (status === "invited") return "border-vega-accent-border bg-vega-accent-soft text-[#93c5fd]";
...
export function UsersAccessWorkspace({ initialUsers, initialRequests, currentUserId }: UsersAccessWorkspaceProps) {
  ...
  return (
    <section className="min-w-0">
      ...
      <div className={cn(panelClass, "mt-4 overflow-hidden")}>
        <div className="flex items-center justify-between gap-3 px-4 py-3"><div className="flex items-center gap-3"><h2 className="text-sm font-semibold text-vega-text">Password requests</h2><span className="rounded-full bg-[#1a2a3c] px-3 py-1 text-[10px] text-vega-text-secondary">{pendingRequests} pending</span></div><button type="button" onClick={() => setPasswordHistoryOpen((open) => !open)} className="inline-flex items-center gap-2 text-[11px] font-medium text-vega-accent">{passwordHistoryOpen ? "Hide history" : "View history"}<ChevronRight className={cn("h-3.5 w-3.5 transition-transform", passwordHistoryOpen && "rotate-90")} /></button></div>
