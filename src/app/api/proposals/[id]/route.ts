...
export async function GET(_: Request, { params }: { params: Params }) {
  try {
    ...
    const proposal = await ProposalModel.findOne({ _id: id, ...await relatedLeadFilter(actor) })
      .populate("leadId")
      .populate("clientId")
...
export async function PATCH(request: Request, { params }: { params: Params }) {
  try {
    ...
    if (status === "signed") {
      ...
      // Revenue must be captured explicitly from the lead's Closed Won form.
      await LeadModel.updateOne({ _id: proposal.leadId, status: { $ne: "closed_won" } }, { $set: { status: "negotiation" } });
    }
